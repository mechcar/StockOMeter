...
app.summaryURL = "https://apidojo-yahoo-finance-v1.p.rapidapi.com/market/v2/get-quotes?region=US";
...
app.getSelectValue = function(){
    $('form').on('submit', function(e){
        ...
        if (app.symbol.length > 5){
            ...
            Swal.fire({
                title: `Error!`,
                text: `It appears that ${app.symbol} is not a listed US security. Most symbols traded in the US have a symbol length between 1 and 5 characters. Did you mean ${app.symbol.slice(0,5)}?`,
                icon: 'warning',
            ...
            $('.errorHandling').text(`It appears that ${app.symbol} is not a listed US security. Most symbols traded in the US have a symbol length between 1 and 5 characters. Did you mean ${app.symbol.slice(0,5)}?`);
...
app.getSummary = function(){
    ...
    .then(function(data){
        ...
        if (typeof values === 'undefined') {
            ...
            $('.symbol').text(`${app.symbol}`);
            $('.errorHandling').text(`It appears that ${app.symbol} is not a listed US security. Try another symbol.`);
            $('.errorHandling').addClass('show');
            $('.errorHandling').removeClass('hidden');
            $('.profileInformation').addClass('show');
            $('.profileInformation').removeClass('hidden');
        ...
        if(!(validExchanges.includes(exchange))) {
            ...
            $('.symbol').text(`${app.symbol}`);
            $('.errorHandling').text(`It appears that ${app.symbol} is not a listed US security. It trades on ${exchange}. Try another symbol.`);
            $('.quotesContent').addClass('hidden');
            $('.quotesContent').removeClass('show');
            $('.chart').addClass('hidden');
            $('.chart').removeClass('show');
        ...
        // If the entire array is comprised of the above values, then the symbol is considered inactive
            if (app.check == 12) {
                ...
                $('.symbol').text(`${app.symbol}`);
                $('.error').text(`It appears that ${app.symbol} is not an active US security. Try another symbol.`);
                $('.errorHandling').addClass('show');
                $('.errorHandling').removeClass('hidden');
                $('.profileInformation').addClass('show');
                $('.profileInformation').removeClass('hidden');  
            ...
            app.populate = function(){
                    ...
                    if (typeof value === 'number') {
                        ...
                        if (value[0] === '$') {
                            if ((parseFloat(value.slice(1,))) < 1){
                                return `$${parseFloat(value.slice(1,)).toFixed(3)}`
                            }
                            else {
                                return `$${parseFloat(value.slice(1,)).toFixed(2)}`
                            }
                        }
                        else if (value[end] === '%') {
                            if (parseFloat(value.slice(0, end)) < 1){
                                return `${parseFloat(value.slice(0, end)).toFixed(3)}%`
                            }
                            else {
                                return `${parseFloat(value.slice(0, end)).toFixed(2)}%`
                            }
                        }
                    }  
                }
                ...
                availableValuesObj.price = app.bracketWrapNegatives(app.pennyStockFormatter(availableValuesObj.price), '$');
                ...
                $('.previousClosingPrice').text(`$${availableValuesObj.previousClosingPrice}`);
                $('.openingPrice').text(`$${availableValuesObj.openingPrice}`);
                $('.daysChangeDollars').text(availableValuesObj.daysChangeDollars);
                $('.daysChangePercent').text(availableValuesObj.daysChangePercent);
                ...
                $('.daysRange').text(`$${availableValuesObj.regularMarketDayLow} - $${availableValuesObj.regularMarketDayHigh}`);
                $('.fiftyTwoWeekRange').text(`$${availableValuesObj.fiftyTwoWeekLow} - $${availableValuesObj.fiftyTwoWeekHigh}`);
                $('.regularMarketVolume').text(`${availableValuesObj.regularMarketVolume.toLocaleString('en-US')}`);
                $('.averageVolume10Day').text(`${availableValuesObj.averageVolume10Day.toLocaleString('en-US')}`);
            ...
            if (app.marketOpen() == 'open'){
                ...
                $('.regularMarketPrice').text(`${availableValuesObj.price}`); 
            }
...
app.getChart = function(){
    ...
    then(function(data){
        ...
        // Error 6: handling symbols without graph timestamp, price or summary data
            ...
            $('.symbol').text(`${app.symbol}`);
            $('.errorHandling').text(`It appears that ${app.symbol} is not an active US security. It may have been delisted. Try another symbol.`);
            $('.errorHandling').addClass('show');
            $('.errorHandling').removeClass('hidden');
            $('.profileInformation').addClass('show');
            $('.profileInformation').removeClass('hidden');
    ...
    options = {
        ...
        vAxis: {
            ...
            title: 'Price',  titleTextStyle: {color: 'white'}, 
            textStyle: {color: 'white'}},
...
app.getProfile = function(){
    ...
    then(function(data){
        ...
        if (app.companyDescription === undefined) {
            ...
            const HTMLtoAppend =
                    `<button class="getProfile" value=${app.symbol}>View Profile</button>`;
